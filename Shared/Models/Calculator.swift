...
enum Calculator {
    ...
    @discardableResult
    func apply(item: CalculatorButtonItem) -> Calculator {
        switch item {
        case .digit(let num):
    ...
    private func apply(num: Int) -> Calculator {
        switch self {
        case .left(let left):
    ...
    private func applyDot() -> Calculator {
        switch self {
        case .left(let left):
    ...
    private func apply(op: CalculatorButtonItem.Op) -> Calculator {
        switch self {
        case .left(let left):
            switch op {
    ...
    private func apply(command: CalculatorButtonItem.Command) -> Calculator {
        switch command {
        case .clear:
            return .left("0")
        case .flip:
